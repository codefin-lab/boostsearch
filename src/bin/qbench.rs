...
fn main() -> anyhow::Result<()> {
    let data = std::env::var("VELOSEARCH_DATA").unwrap_or("/tmp/startup-data".into());
    ...
    for (name, body) in &queries {
        ...
        for _ in 0..5 {
            let _ = velosearch::search::run(&store, &index, body, &params);
        }
        let t = Instant::now();
        let mut hits = 0;
        for _ in 0..n {
            match velosearch::search::run(&store, &index, body, &params) {
                Ok(o) => hits = o.total,
                Err(_) => {
