...
//! counts a quarter -- and VeloCore's own BM25 takes only whole counts, so
...
impl Similarity {
    ...
    pub(crate) fn new(
        ...
        probe: &Term,
        terms: &[Term],
    ) -> velocore::Result<Option<Similarity>> {
        ...
        for (id, slot) in cache.iter_mut().enumerate() {
            let dl = velocore::fieldnorm::FieldNormReader::id_to_fieldnorm(id as u8) as f32;
...
pub(crate) fn norms_for(reader: &velocore::SegmentReader, probe: &Term) -> velocore::Result<Norms> {
...
pub(crate) struct Norms(Option<velocore::fieldnorm::FieldNormReader>);

impl Norms {
    pub(crate) fn fieldnorm_id(&self, doc: velocore::DocId) -> u8 {
...
pub(crate) fn docs_of(
    ...
    term: &Term,
) -> velocore::Result<Vec<velocore::DocId>> {
...
pub(crate) fn intersect(a: &[velocore::DocId], b: &[velocore::DocId]) -> Vec<velocore::DocId> {
...
pub(crate) fn union(a: &[velocore::DocId], b: &[velocore::DocId]) -> Vec<velocore::DocId> {
...
pub(crate) struct SegmentPositions {
    postings: Vec<Option<velocore::postings::SegmentPostings>>,
    buffer: Vec<u32>,
}
...
impl SegmentPositions {
    pub(crate) fn open(
        reader: &velocore::SegmentReader,
        terms: &[Term],
    ) -> velocore::Result<SegmentPositions> {
    ...
    pub(crate) fn read(&mut self, doc: velocore::DocId, out: &mut Vec<Vec<i32>>) {
