...
Sixteen use cases. Each one is **a project of its own**: its own documentation,
...
fourteen that can share it, in order. 06 (its own security-enabled node) and 13
(its own cluster) are skipped, and their READMEs say how to run them.
