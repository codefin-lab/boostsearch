...
In that case, `boostcore-columnar`'s behavior is as follows:
...
from Number or boolean values. `boostcore-columnar` will simply emit several columns associated to a given column_name.
...
`boostcore-columnar` will pick the first type that can represents the set of appended value, with the following prioriy order (`i64`, `u64`, `f64`).
