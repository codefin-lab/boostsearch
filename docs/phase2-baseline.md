...
## Phase 3 รอบ 96.5% → 99.5% ไม่กระทบความเร็ว

วัดแบบสลับ old/new สามรอบติดกัน (vus=8, 15s, mix เดิม: match_all / term /
terms agg) บน dataset เดิม:

| | รอบ 1 | รอบ 2 | รอบ 3 | เฉลี่ย |
|---|---:|---:|---:|---:|
| ก่อน (40f6778) | 92,833 | 92,555 | 92,502 | 92,630 |
| หลัง | 92,464 | 92,288 | 92,833 | 92,528 |

ต่างกัน −0.1% ซึ่งอยู่ในระดับ noise ของการวัด · ของที่เพิ่มเข้าไปในเส้นทาง search
รอบนี้ (จดว่า field ไหนโหลด fielddata, peel percentiles, date_histogram ที่มี
time zone) ไม่มีตัวไหนอยู่บนเส้นทางที่ร้อน

## หลังปิด compatibility ถึง 99.1% — ช้าลง 2%

วัดสลับ old/new สามรอบ (vus=8, 15s, mix เดิม) บน dataset เดิม:

| | รอบ 1 | รอบ 2 | รอบ 3 | เฉลี่ย |
|---|---:|---:|---:|---:|
| ก่อน (40f6778) | 91,201 | 92,129 | 91,583 | 91,638 |
| หลัง | 89,057 | 89,790 | 90,096 | 89,648 |

−2.2% · ไล่หาต้นเหตุแล้ว **ไม่ได้มาจากจุดเดียว**: fieldnorms ทดสอบแยกแล้วไม่ใช่
(dataset เดิมไม่มี norms อยู่แล้ว) · ที่เหลือคือสิ่งที่เพิ่มเข้ามาในเส้นทาง search
ทีละนิด — เพดานต่าง ๆ ที่ต้องตรวจ, การจดว่า field ไหนโหลด fielddata,
post-pass ของ aggregation, กฎของ collapse/rescore

ลดไปได้บางส่วนแล้วด้วยการเดินต้นไม้ query **รอบเดียว** แทนหกรอบ (แต่ละฟีเจอร์
