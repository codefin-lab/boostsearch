...
## ผล (ล่าสุด — วัดแบบสลับฝั่ง)

⚠️ **แก้จากที่รายงานไว้ก่อนหน้า** — รอบก่อนวัดฝั่งละ 5 รอบติดกัน ซึ่งบนเครื่องที่โหลด
เปลี่ยนตลอดเวลาทำให้ผลเอนตามช่วงเวลาที่วัด รอบนี้ **สลับ obsearch/OpenSearch ทีละรอบ 4 รอบ**
เพื่อหักล้าง drift และผลออกมาต่างจากเดิมมาก

| | obsearch | OpenSearch 3.1.0 | |
|---|---:|---:|---|
| **memory (MB)** | **403** | 1,765 | obsearch **4.4 เท่า** |
| index docs/s | 74,329 | 72,532 | เสมอ (+2%) |
| qps c=1 | 417 | 414 | เสมอ |
| p50 c=1 | 2.29 ms | 2.27 ms | เสมอ |
| p99 c=1 | 3.78 ms | 3.77 ms | เสมอ |
| qps c=8 | 1,771 | **1,861** | **OpenSearch +5%** |
| p50 c=8 | 4.25 ms | **3.99 ms** | OpenSearch |
| p99 c=8 | 6.85 ms | **6.06 ms** | **OpenSearch +13%** |
...
|---|---:|---:|---|
| match_text | **2.20** | 2.66 | obsearch |
| bool_filter | **2.48** | 2.75 | obsearch |
| sort_paged | **2.47** | 2.79 | obsearch |
| range_numeric | **2.16** | 2.34 | obsearch |
| time_range | **2.21** | 2.24 | obsearch |
| term_numeric | **2.08** | 2.12 | obsearch |
| match_all | 2.29 | **2.08** | OpenSearch |
| term_keyword | 2.22 | **2.01** | OpenSearch |
| agg_terms | 2.43 | **2.12** | OpenSearch |
| agg_date_hist | 2.49 | **2.21** | OpenSearch |
| agg_nested | 2.90 | **2.08** | OpenSearch |
| time_range_agg | 2.60 | **2.08** | OpenSearch |
